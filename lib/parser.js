 ...
 * - Timezone support via TIMEZONE env var
 */
...
// Timezone offset in minutes (default to IST +5:30 = 330 minutes)
// Set TIMEZONE env var to override (e.g., "330" for IST, "0" for UTC, "-300" for EST)
const USER_TIMEZONE_OFFSET = parseInt(process.env.TIMEZONE || '330');
...
function preprocessText(text) {
  ...
  // "X weekly" or "weekly X" pattern (e.g., "monday weekly", "weekly monday")
  // Convert to "every monday" pattern
  const weeklyDayMatch = normalized.match(/\b(mon(?:day)?|tue(?:s(?:day)?)?|wed(?:nesday)?|thu(?:r(?:s(?:day)?)?)?|fri(?:day)?|sat(?:urday)?|sun(?:day)?)\s+weekly\b/i);
  if (weeklyDayMatch) {
    normalized = normalized.replace(/\b(mon(?:day)?|tue(?:s(?:day)?)?|wed(?:nesday)?|thu(?:r(?:s(?:day)?)?)?|fri(?:day)?|sat(?:urday)?|sun(?:day)?)\s+weekly\b/i, 'every $1');
  }
  
  // Also handle "weekly monday" → "every monday"
  const weeklyBeforeDayMatch = normalized.match(/\bweekly\s+(mon(?:day)?|tue(?:s(?:day)?)?|wed(?:nesday)?|thu(?:r(?:s(?:day)?)?)?|fri(?:day)?|sat(?:urday)?|sun(?:day)?)\b/i);
  if (weeklyBeforeDayMatch) {
    normalized = normalized.replace(/\bweekly\s+(mon(?:day)?|tue(?:s(?:day)?)?|wed(?:nesday)?|thu(?:r(?:s(?:day)?)?)?|fri(?:day)?|sat(?:urday)?|sun(?:day)?)\b/i, 'every $1');
  }
...
function formatDateForTickTick(date, hasTime = false) {
  ...
  // Use user's timezone offset instead of server's
