...
IMPORTANT RULES:
1. EXPLICIT TIME WINS: If task has specific time like "10am", "3pm", "7:30pm", "14:00" → use "time" field (HH:MM), set timeOfDay to NULL
2. timeOfDay ONLY for vague: Only use timeOfDay for words like "morning", "evening", "tonight", "afternoon" when NO specific time given
3. KEEP ALL NUMBERS IN TITLE: Numbers like "100", "50", quantities must stay in title (e.g., "skipping 100" → title: "skipping 100")
4. Title should contain the core task description without date/time/recurring keywords

Return ONLY valid JSON (no markdown, no explanation):
{
  "title": "clean task title - KEEP numbers and quantities",
  "date": "YYYY-MM-DD or null if no specific date",
  "time": "HH:MM in 24h format or null - USE THIS for explicit times like 10am, 3pm",
  "timeOfDay": "morning | afternoon | evening | night | null - ONLY if no explicit time",
...
Examples:
- "skipping 100 daily at 10am" → title: "skipping 100", time: "10:00", timeOfDay: null, recurring: {frequency:"daily"}
- "buy milk tomorrow 5pm" → title: "buy milk", date: tomorrow's date, time: "17:00", timeOfDay: null
- "gym every mon wed fri 7am" → title: "gym", time: "07:00", timeOfDay: null, recurring: {frequency:"weekly", daysOfWeek:["mon","wed","fri"]}
- "meeting every 2 weeks" → title: "meeting", recurring: {frequency:"weekly", interval:2}
- "call mom day after tomorrow evening" → title: "call mom", date: 2 days from now, timeOfDay: "evening", time: null
- "workout in the morning" → title: "workout", timeOfDay: "morning", time: null
- "dinner tonight" → title: "dinner", timeOfDay: "night", date: today, time: null
- "read 50 pages daily" → title: "read 50 pages", recurring: {frequency:"daily"}
- "standup weekdays 10am" → title: "standup", time: "10:00", timeOfDay: null, recurring: {frequency:"weekly", daysOfWeek:["mon","tue","wed","thu","fri"]}`;
...
async function parseTask(text) {
  ...
  if (aiResult) {
    ...
    // PRIORITY: Explicit time ALWAYS wins over timeOfDay
    // Only convert timeOfDay if NO explicit time was given
    if (aiResult.time) {
      // Explicit time exists - ignore any timeOfDay
      console.log(`[AI] Using explicit time: ${aiResult.time} (ignoring timeOfDay if any)`);
    } else if (aiResult.timeOfDay && TIME_OF_DAY[aiResult.timeOfDay.toLowerCase()]) {
      // No explicit time - convert timeOfDay to time
      ...
      console.log(`[AI] No explicit time, converted timeOfDay "${aiResult.timeOfDay}" to time ${aiResult.time}`);
    }
    ...
    if (result.duration && result.dueDate) {
      ...
      // No duration: startDate = dueDate (TickTick expects both to be the same)
      result.startDate = result.dueDate;
    }

    // Set isAllDay only when we have a date
    if (result.dueDate) {
